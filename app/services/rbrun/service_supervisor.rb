...
module Rbrun
  ...
  class ServiceSupervisor
    ...
    # Recent output of a service — a non-follow SNAPSHOT. Never stream here: a live service's log stream
    # never closes, so a follow-based tail hangs the caller (and the whole turn) indefinitely.
    def tail(run, lines: 200)
      return "" if run.process_session.blank? || run.cmd_id.blank?

      out = @sandbox.session_logs(run.process_session, run.cmd_id).to_s
